...
async function shopifyAdminApiRequest({ endpoint, method = 'GET', body }: { endpoint: string; method?: string; body?: object }): Promise<any> {
    ...
    // This tool now calls the Vercel serverless function proxy.
    const url = '/api/shopify';
    try {
        const response = await fetch(url, {
            method: 'POST',
        ...
        if (!response.ok) {
            return {
                error: {
                    message: `Shopify API proxy request failed with status ${response.status}`,
                    status: response.status,
                    body: responseBody,
                    requestArgs,
                    suggestion: "Check the Vercel function logs and ensure environment variables (LUMINOUS_SHOPIFY_STORE_URL, LUMINOUS_SHOPIFY_ADMIN_TOKEN) are set correctly in the Vercel project."
                }
            };
        }
        ...
        console.error(`[Tool: shopifyAdminApiRequest] Fetch failed for proxy URL: ${url}`, e);
        const errorMessage = e instanceof Error ? e.message : String(e);
        return {
            error: {
                message: `The Shopify proxy request failed to complete. This could be a local network issue or a problem with the Vercel deployment.`,
                details: errorMessage,
                requestArgs,
                suggestion: "Verify network connectivity and check the status of the Vercel deployment."
            }
        };
    }
}
